...
export default async function DashboardPage({ params }: { params: Promise<{ siteId: string }> }) {
  const { siteId } = await params;
  const cookieStore = await cookies();
  const lang = (cookieStore.get("admin-lang")?.value || "mn") as "mn" | "en";
  const t = adminTranslations[lang];

  const statsResult = await fetchAdminStats(siteId);
  ...
  try {
    const ordersRes = await apiGet<{ data: Order[] }>(`/api/v1/admin/orders?limit=5&lang=${lang}&siteId=${siteId}`);
  ...
  try {
    const convRes = await apiGet<{ data: Conversation[] }>(`/api/v1/admin/conversations?limit=5&lang=${lang}&siteId=${siteId}`);
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-8">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <SectionCard title={t.dashboard.recentOrders.title} href={`/${siteId}/orders`} actionLabel={t.dashboard.recentOrders.viewAll}>
        ...
        <SectionCard title={t.dashboard.recentChats.title} href={`/${siteId}/chat`} actionLabel={t.dashboard.recentOrders.viewAll}>
